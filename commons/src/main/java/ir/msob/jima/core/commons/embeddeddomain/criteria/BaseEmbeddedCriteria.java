...
/**
 * The {@code BaseEmbeddedCriteria} interface represents the basic class for embedded domain criteria models.
 * It extends the {@code BaseCriteria} interface with a generic type {@code BaseEmbeddedCriteria<ID>},
 * allowing embedded domain criteria models to be compared based on their IDs.
 * <p>
 * This interface is generic, with the generic type {@code ID} extending {@code Comparable} and {@code Serializable}.
 * It means that the ID of the embedded domain criteria model can be of any type that is comparable and serializable.
 * <p>
